...
class Image_moo
{
	...
	//----------------------------------------------------------------------------------------------------------
	{
		...
		// Enable interlancing
		imageinterlace($this->temp_image, true);
	...
	//----------------------------------------------------------------------------------------------------------
	{
        ...
        imagesavealpha($this->temp_image, true);
        $color = imagecolorallocatealpha($this->temp_image, 0, 0, 0, 127);
        imagefilledrectangle($this->temp_image, 0, 0, $mw, $mh, $color);
	...
	//----------------------------------------------------------------------------------------------------------
	{
        ...
        //$col = $this->_html2rgb($this->background_colour);
        //$bg = imagecolorallocate($this->temp_image, $col[0], $col[1], $col[2]);
        //imagefilledrectangle($this->temp_image, 0, 0, $x2-$x1, $y2-$y1, $bg);

		//Get the file extension
		$ext = strtolower(pathinfo($this->filename, PATHINFO_EXTENSION));

		//Has transparency.
		//TODO: use $this->_hasAlpha($this->temp_image) for better results
		...
		imagecopy($this->temp_image, $this->main_image, -$x1, -$y1, 0, 0, $this->width, $this->height);
	 ...
	 * It resizes the image and checks pixel by pixel to find one that's transparent
	 *
	 * @param $imgdata
	 * @return bool
	 */
	private function _hasAlpha($imgdata) {
		$w = imagesx($imgdata);
		$h = imagesy($imgdata);
