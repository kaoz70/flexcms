...
<body>
	...
	<div id="header">
        ...
        <ul id="menu" class="contenido">
	        <li><a title="Estructura" rel="Crear páginas, editar su estructura, añadir módulos" class="nivel1 ajax tooltip" data-controller="admin/structure" href="<?=base_url('admin/structure');?>"><i class="material-icons">view_quilt</i>Estructura</a></li>
            <? foreach ($menu as $item): ?>
                <li><a title="<?=$item->name->es?>" rel="<?=$item->tooltip->es?>" class="nivel1 ajax tooltip" data-controller="<?=$item->controller?>" href="<?=base_url($item->controller);?>"><i class="material-icons"><?=$item->icon?></i><?=$item->name->es?></a></li>
            <? endforeach ?>
	        <li><a title="Idiomas" rel="Editar idiomas para sitios multi-idiomas" class="nivel1 ajax tooltip" data-controller="admin/lang" href="<?=base_url('admin/lang');?>"><i class="material-icons">language</i>Idiomas</a></li>
	        <li><a title="Configuración" rel="Tamaños de imagenes, configuracion general" class="nivel1 ajax tooltip" data-controller="admin/config" href="<?=base_url('admin/config');?>"><i class="material-icons">settings</i>Configuraci&oacute;n</a></li>
        </ul>
        <div class="user">
            <div id="counter"></div>
            <a title="Cerrar sessi&oacute;n" rel="<?=$user->first_name?> <?=$user->last_name?>" class="external tooltip" id="logout" href="<?=base_url();?>admin/logout"><span></span>Salir</a>
        </div>
	</div>
