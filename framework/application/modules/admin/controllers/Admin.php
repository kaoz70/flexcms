...
class Admin extends AdminController {
	...
	public function index()
	{
		...
		$activationRepo = new \Cartalyst\Sentinel\Activations\IlluminateActivationRepository();

		//Create an activation TODO send by email
		$activation = $activationRepo->create($user);

		//Activate the user TODO get code by email
		$activationRepo->complete($user, $activation->code);
		...
		*/
		...
		$role = $roles->createModel()->create($input);*/
		...
		if (Sentinel::check() AND (Sentinel::inRole('admin') || Sentinel::inRole('superadmin'))) {
			$this->renderAdmin();
		}

		//User is logged in but not admin
		else if (Sentinel::check() AND (!Sentinel::inRole('admin') || !Sentinel::inRole('superadmin'))) {
			$data['error'] = 'Usted no tiene los permisos necesarios para poder entrar a esta secci&oacute;n. <a href="'. base_url() .'">Salir</a>';
	...
	public function validate(){
		...
		if ($this->form_validation->run() == true) {
			...
			try {
				...
				if (Sentinel::authenticate($credentials, (bool) $this->input->post('remember'))) {

					if ( $this->input->is_ajax_request() AND (Sentinel::inRole('admin') || Sentinel::inRole('superadmin'))) {
