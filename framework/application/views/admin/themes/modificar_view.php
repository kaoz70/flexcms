	...
	<table id="pageData">
		<tr>
			<td>
				...
				echo form_open('admin/themes/modificar', $attributes);
				?>
				...
				<div class="field" id="backgrounds">
					...
					<div class="content">
						...
						<fieldset id="upload-backgrounds">
							...
							<ul class="list galeria" id="theme_backgrounds" style="overflow: hidden" data-sort="<?=base_url('admin/theme/reorganizar/'.$folder)?>">
								...
								<? foreach ($config->backgrounds as $background):?>
									...
									<li class="image drag" id="<?=$background?>">
										...
										<a href="<?= base_url('admin/theme/delete_background/' . $folder . '/' . $background) ?>" class="eliminar" >eliminar</a>
									</li>
...
<a href="<?= base_url('admin/theme/actualizar/') ?>" data-level="nivel2" class="guardar boton importante n1" >Modificar</a>

<script type="text/javascript">
    upload.image('upload-image-logo', 'logo_extension', '<?=base_url();?>admin/theme/logo/<?=$folder?>', 0, 0, false);
	upload.gallery('upload-backgrounds', 'theme/<?=$folder?>', 0, 0, '', '', '<?= base_url('admin/theme/delete_background/' . $folder . '/') ?>');
