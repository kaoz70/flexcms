<h3 id="tempMod_<?=$moduleId?>">Articulo</h3>
...
<p class="input small">
	<label for="parametro1">Artículo:</label>
	<select id="parametro1" name="parametro1">
		<? foreach ($articulos as $key => $value): ?>
			<option <?= $value['articuloId'] == $moduleData->moduloParam1 ? 'selected' : '' ?>  value="<?=$value['articuloId']?>"><?=$value['articuloTitulo']?></option>
		<? endforeach ?>
	</select>
</p>
...
<input type="hidden" name="moduloVerPaginacion" value="off" />
